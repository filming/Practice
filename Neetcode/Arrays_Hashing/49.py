...
- We can use a fixed size length array/list in order to avoid sorting algorithm time.
'''
...
class Solution:
	...
	def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
		...
		for curr_word in strs:
			curr_word_count = [0] * 26

			for curr_char in curr_word:
				curr_word_count[ord(curr_char) - ord('a')] += 1
			
			curr_word_count_tuple = tuple(curr_word_count)
			
			if curr_word_count_tuple in result:
				result[curr_word_count_tuple].append(curr_word)
			else:
				result[curr_word_count_tuple] = [curr_word]
